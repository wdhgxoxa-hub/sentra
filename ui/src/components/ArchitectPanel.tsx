...
export function ArchitectPanel({ clusterKey }: { clusterKey: string }) {
  ...
  useEffect(() => {
    const unlisten = onArchitectChunk((chunk) => {
      if (chunk.clusterKey !== clusterKey || chunk.done) return;
    ...
    <section className="rounded-card border border-border bg-surface">
      <div className="flex flex-wrap items-center gap-2 border-b border-border px-4 py-3">
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg bg-accent px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-accent-hover disabled:opacity-40"
        >
          {generando ? (
