...
Reglas: cuota en unidades con el presupuesto de D-M4 (2.000 por escaneo);
...
def api(busqueda=None, hilos=None, peticiones=None):
...
class TestBusqueda(unittest.IsolatedAsyncioTestCase):
    async def test_busca_videos_y_lee_sus_comentarios_cobrando_unidades(self):
        ...
        hilos = parse_qs(urlparse(str(peticiones[1].url)).query)
        self.assertEqual(hilos["videoId"], ["vidInvent01"])
        self.assertEqual([i.kind for i in items], ["post", "comment"])
        self.assertEqual(adaptador.budget.spent_units, 101)

    async def test_video_y_comentario_sin_nombres_y_con_urls_por_id(self):
        video, comentario = await todos(fuente(api()).search(SearchQuery(keywords=["invoice"])))
        self.assertEqual(video.id, "youtube:vidInvent01")
        self.assertEqual(video.url, "https://www.youtube.com/watch?v=vidInvent01")
        self.assertEqual(video.title, "Why is invoicing still so painful?")
        ...
        for item in (video, comentario):
            self.assertEqual(item.community, "YouTube")
    ...
    async def test_comentarios_desactivados_no_paran_la_fuente(self):
        ...
        self.assertEqual([i.kind for i in items], ["post"])
